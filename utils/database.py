# utils/database.py (자동 역할 기능 추가, 생략 없음)
...
try:
    ...
    logger.error("❌ Supabase Async Client Creation Failed: %s",
                 e,
                 exc_info=True)
...
def get_role_id(key: str) -> int | None:
    ...
    if role_id is None:
        logger.warning(
            f"[get_role_id] Role ID for key '{key}' is not set in ROLE_ID_CONFIG."
        )
    return role_id
...
async def get_counter_configs():
    ...
    try:
        response = await supabase.table('channel_counters').select('*'
                                                                   ).execute()
...
async def add_counter_config(channel_id: int,
                             guild_id: int,
                             counter_type: str,
                             format_string: str,
                             role_id: int = None):
    if not supabase: return
    try:
        await supabase.table('channel_counters').upsert(
            {
...
async def remove_counter_config(channel_id: int):
    if not supabase: return
    try:
        await supabase.table('channel_counters').delete().eq(
            'channel_id', channel_id).execute()
    except Exception as e:
        logger.error(f"[DB Error] remove_counter_config: {e}", exc_info=True)


# --- 자동 역할 버튼 관리 함수 ---
async def add_auto_role_button(message_id: int, role_id: int, label: str,
                               emoji: str | None, style: str):
    if not supabase: return
    try:
...
async def remove_auto_role_button(message_id: int, role_id: int):
    if not supabase: return
    try:
        await supabase.table('auto_roles').delete().match({
            'message_id': message_id,
            'role_id': role_id
        }).execute()
    except Exception as e:
...
async def get_auto_role_buttons(message_id: int):
    if not supabase: return []
    try:
        response = await supabase.table('auto_roles').select('*').eq(
...
# --- 기존 데이터 관리 함수들 ---
async def get_or_create_user(table_name: str, user_id_str: str,
                             default_data: dict):
    if not supabase:
        logger.error(
            f"[DB Error] Supabase client is not initialized (get_or_create_user - {table_name})."
        )
        return {}
    try:
        select_response = await supabase.table(table_name).select("*").eq(
            "user_id", user_id_str).limit(1).execute()
        if select_response.data:
            return select_response.data[0]
        else:
            insert_data = {"user_id": user_id_str, **default_data}
            insert_response = await supabase.table(table_name).insert(
                insert_data, returning="representation").execute()
            if insert_response.data: return insert_response.data[0]
            else: return {}
    except Exception as e:
        logger.error(f"[DB Error] get_or_create_user on '{table_name}': {e}",
                     exc_info=True)
        return {}


async def get_wallet(user_id: int):
    return await get_or_create_user('wallets', str(user_id),
                                    {"balance": 0}) or {
                                        "balance": 0
                                    }
...
async def update_wallet(user: discord.User, amount: int):
    ...
    try:
        ...
        response = await supabase.rpc('increment_wallet_balance',
                                      params).execute()
...
async def get_inventory(user_id_str: str):
    if not supabase: return {}
    try:
        response = await supabase.table('inventories').select(
            'item_name, quantity').eq('user_id',
                                      user_id_str).gt('quantity', 0).execute()
...
async def get_aquarium(user_id_str: str):
    if not supabase: return []
    try:
        response = await supabase.table('aquariums').select(
            'id, name, size, emoji').eq('user_id', user_id_str).execute()
...
async def remove_fish_from_aquarium(fish_id: int):
    ...
    try:
        ...
        logger.error(f"[DB Error] remove_fish_from_aquarium: {e}",
                     exc_info=True)


async def get_user_gear(user_id_str: str):
    if not supabase: return {"rod": "素手", "bait": "エサなし"}
    gear = await get_or_create_user('gear_setups', user_id_str, {
        "rod": "古い釣竿",
        "bait": "エサなし"
    }) or {}
...
async def set_user_gear(user_id_str: str, rod: str = None, bait: str = None):
    if not supabase: return
    try:
        await get_or_create_user('gear_setups', user_id_str, {
            "rod": "古い釣竿",
            "bait": "エサなし"
        })
        data_to_update = {}
        ...
        if data_to_update:
            await supabase.table('gear_setups').update(data_to_update).eq(
                'user_id', user_id_str).execute()
    except Exception as e:
...
async def save_panel_id(name: str, mid: int):
    ...
    try:
        await supabase.table('panel_data').upsert(
            ...
            on_conflict="panel_name").execute()
    except Exception as e:
...
async def get_panel_id(name: str) -> int | None:
    if not supabase: return None
    try:
        res = await supabase.table('panel_data').select('message_id').eq(
            'panel_name', name).limit(1).execute()
...
async def save_channel_id_to_db(channel_key: str, object_id: int):
    ...
    try:
        await supabase.table('channel_configs').upsert(
            ...
            on_conflict="channel_key").execute()
...
async def get_all_channel_configs():
    ...
    if not supabase: return {}
    try:
        response = await supabase.table('channel_configs').select(
            'channel_key, channel_id').execute()
        _cached_channel_configs = {
            item['channel_key']: item['channel_id']
...
async def get_channel_id_from_db(channel_key: str) -> int | None:
    ...
    try:
        response = await supabase.table('channel_configs').select(
            'channel_id').eq('channel_key', channel_key).limit(1).execute()
        if response.data:
...
async def get_activity_data(user_id_str: str):
    return await get_or_create_user('activity_data', user_id_str, {
        "chat_counts": 0,
        "voice_minutes": 0
    }) or {}


async def update_activity_data(user_id_str: str,
                               chat_increment=0,
                               voice_increment=0,
                               reset_chat=False,
                               reset_voice=False):
...
async def get_cooldown(user_id_str: str) -> float:
    ...
    try:
        response = await supabase.table('cooldowns').select(
            'last_cooldown_timestamp').eq('user_id',
                                          user_id_str).limit(1).execute()
        if response.data and response.data[0][
                'last_cooldown_timestamp'] is not None:
...
CURRENCY_ICON = "🪙"
ROLE_PREFIX_MAPPING = {
    933077535405789205: "一",
    933077534994755654: "二",
    933077536253050970: "三",
    933077542699663390: "四",
    1209471813319528468: "五",
    1209471819866841149: "六",
    1209471820559032320: "七",
    1209471821166944266: "八",
    1209471821632770099: "九",
}
ROD_HIERARCHY = ["古い釣竿", "カーボン釣竿", "専門家用の釣竿", "伝説の釣竿"]

ITEM_DATABASE = {
    "住人票Lv.1": {
        "id": 1404444315383500811,
        "price": 100,
        "category": "里の役職",
        "description": "基本的な特典が含まれています。",
        "emoji": "1️⃣",
        "buyable": True,
        "sellable": False
    },
    "住人票Lv.2": {
        "id": 1404444316021035019,
        "price": 50,
        "category": "里の役職",
        "description": "追加のチャンネルへのアクセスが可能になります。",
        "emoji": "2️⃣",
        "buyable": True,
        "sellable": False
    },
    "仮住人": {
        "id": 1404444316843376721,
        "price": 10,
        "category": "里の役職",
        "description": "里の雰囲気を体験できます。",
        "emoji": "🧑‍🌾",
        "buyable": True,
        "sellable": False
    },
    "一般の釣りエサ": {
        ...
        "buyable": True,
        "sellable": True,
        "description": "魚のアタリが来るまでの時間を短縮します。(5～10秒)",
        "emoji": "🐛",
        "bite_time_range": (5.0, 10.0)
    },
    "高級釣りエサ": {
        "price": 20,
        "sell_price": 10,
        ...
        "buyable": True,
        "sellable": True,
        "description": "魚のアタリが来るまでの時間を大幅に短縮します。(4～8秒)",
        "emoji": "✨",
        "bite_time_range": (4.0, 8.0)
    },
    "古い釣竿": {
        "price": 100,
        "category": "釣り",
    ...
    "カーボン釣竿": {
        ...
        "sellable": False,
        "buyable": True,
        "description": "珍しい魚が釣れる確率が少し増加します。(+5%)",
        "emoji": "🎣",
        "good_fish_bonus": 0.05,
        "is_upgrade_item": True
    },
    "専門家用の釣竿": {
        "price": 20000,
        ...
        "sellable": False,
        "buyable": True,
        "description": "珍しい魚が釣れる確率が目に見えて増加します。(+10%)",
        "emoji": "🏆",
        "good_fish_bonus": 0.10,
        "is_upgrade_item": True
    },
    "伝説の釣竿": {
        "price": 100000,
        ...
        "sellable": False,
        "buyable": True,
        "description": "伝説の魚も釣れるかもしれません。(+15%)",
        "emoji": "🌟",
        "good_fish_bonus": 0.15,
        "is_upgrade_item": True
    },
    "ジャガイモの種": {
        "price": 10,
        "sell_price": 5,
        "category": "農業",
        "description": "植えるとジャガイモが育ちます。成長時間：1時間",
        "emoji": "🥔",
        "buyable": True,
        "sellable": True
    },
    "ジャガイモ": {
        "sell_price": 25,
        "category": "農業",
        "description": "美味しく料理できる、よく育ったジャガイモです。",
        "emoji": "🥔",
        "buyable": False,
        "sellable": True
    },
    "鶏のエサ": {
        ...
        "sell_price": 4,
        "category": "牧場",
        "description": "鶏に与えると卵を産む可能性があります。",
        "emoji": "🌾",
        "buyable": True,
        "sellable": True
    }
