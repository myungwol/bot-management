...
# --- [신규] 봇 설정값을 DB에 저장하는 함수 ---
@supabase_retry_handler()
async def save_config_to_db(key: str, value: Any):
    """bot_configs 테이블에 설정값을 저장(upsert)합니다."""
...
async def sync_defaults_to_db():
    ...
    try:
        # 1. 역할 맵 동기화
        # UI_ROLE_KEY_MAP에서 'name'만 추출하여 간단한 맵으로 만듦
        ...
        # 'is_prefix'가 True인 역할들을 'priority' 순으로 정렬하여 칭호 목록 생성
...
async def load_game_data_from_db():
    ...
    if loot_response.data:
        _fishing_loot_cache = loot_response.data; logger.info(f"✅ {len(_fishing_loot_cache)}개의 낚시 결과물 정보를 DB에서 로드했습니다.")
...
async def get_or_create_user(table_name: str, user_id_str: str, default_data: dict) -> dict:
    try:
        ...
        logger.error(f"'{table_name}' 테이블에서 유저 데이터 조회/생성 실패. 기본 데이터를 반환합니다: {e}"); return default_data
