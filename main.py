...
from utils.database import load_all_configs_from_db, load_game_data_from_db
...
async def regenerate_all_panels():
    ...
    for cog_name in panel_cogs:
        ...
        if cog and hasattr(cog, 'regenerate_panel'):
            try: await cog.regenerate_panel()
...
async def on_ready():
    ...
    for cog_name, cog in bot.cogs.items():
        if hasattr(cog, 'load_all_configs'): await cog.load_all_configs()
