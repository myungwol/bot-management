...
class ServerLogger(commands.Cog):
    ...
    async def on_guild_role_create(self, role: discord.Role):
        ...
        embed = discord.Embed(title="역할 생성됨", color=discord.Color.green(), timestamp=datetime.now(timezone.utc))
    ...
    async def on_guild_role_delete(self, role: discord.Role):
        ...
        if not user or user.bot: return
        embed = discord.Embed(title="역할 삭제됨", color=discord.Color.dark_red(), timestamp=datetime.now(timezone.utc))
    ...
    async def on_guild_role_update(self, before: discord.Role, after: discord.Role):
        ...
        if not user or user.bot: return
        embed = discord.Embed(title="역할 업데이트됨", color=discord.Color.blue(), timestamp=datetime.now(timezone.utc))
        embed.add_field(name="역할", value=f"{after.mention}", inline=False)
        changes = []
    ...
    async def on_guild_update(self, before: discord.Guild, after: discord.Guild):
        ...
        if not user or user.bot: return
        embed = discord.Embed(title="서버 설정 업데이트됨", color=discord.Color.purple(), timestamp=datetime.now(timezone.utc))
        changes = []
        if before.name != after.name:
