# cogs/games/fishing.py (안정성, UX, 코드 구조 개선 최종본)
...
# --- 상수 정의 ---
BIG_CATCH_THRESHOLD = 70.0 # 이 크기 이상이면 "대어"로 취급
BITE_REACTION_TIME = 3.0 # 입질 후 반응 대기 시간 (초)
...
class FishingGameView(ui.View):
    ...
    async def start_game(self, interaction: discord.Interaction, embed: discord.Embed):
        """게임을 시작하고 메시지를 보낸 뒤, 게임 로직 태스크를 실행합니다."""
    ...
    async def _handle_catch_logic(self) -> tuple[discord.Embed, bool, bool]:
        """잡은 아이템에 따라 결과 Embed를 생성하고, 공개 로그 필요 여부를 반환합니다."""
        ...
        if "min_size" in catch_proto: # 물고기인 경우
            log_publicly = True
            ...
            log_publicly = catch_proto.get("log_publicly", False) # 宝箱 같은 아이템은 공개
    ...
    async def catch_button(self, interaction: discord.Interaction, button: ui.Button):
        ...
        result_embed = None
        log_publicly = is_big_catch = False
    ...
    async def _send_result(self, embed: discord.Embed, log_publicly: bool = False, is_big_catch: bool = False):
        footer_private = f"残りのエサ: 一般({self.remaining_baits.get('一般の釣りエサ', 0)}개) / 高級({self.remaining_baits.get('高級釣りエサ', 0)}개)"
        footer_public = f"使用した装備: {self.used_rod} / {self.used_bait}"
        
        fishing_cog = self.bot.get_cog("Fishing")
        if log_publicly and fishing_cog and (log_ch_id := fishing_cog.fishing_log_channel_id) and (log_ch := self.bot.get_channel(log_ch_id)):
            public_embed = embed.copy()
            public_embed.set_footer(text=footer_public)
            
            # [UX 개선] 대어 낚시 시 사용자 멘션
        ...
        async with lock:
            ...
            try:
                ...
                if rod == "素手" or ITEM_DATABASE.get(rod) is None:
                    raise ValueError("「古い釣竿」以上の釣竿を装備してください。")
...
class Fishing(commands.Cog):
    ...
    async def regenerate_panel(self, channel: Optional[discord.TextChannel] = None):
        target_channel = channel or (self.bot.get_channel(self.fishing_panel_channel_id) if self.fishing_panel_channel_id else None)
