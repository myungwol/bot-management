...
from utils.ui_defaults import CUSTOM_EMBED_SENDER_ROLES # 권한 설정을 위해 새로 추가할 변수

logger = logging.getLogger(__name__)

# 1. 임베드 내용을 입력받는 Modal
class EmbedCreatorModal(ui.Modal, title="カスタム埋め込み作成"):
    title_input = ui.TextInput(label="タイトル", placeholder="埋め込みのタイトルを入力してください。", required=True, max_length=256)
    description_input = ui.TextInput(label="説明", placeholder="埋め込みの説明文を入力してください。", style=discord.TextStyle.paragraph, required=False, max_length=2000)
        ...
        try:
            ...
            if self.color_input.value:
                try:
                    ...
                    await interaction.followup.send("❌ 色コードが無効です。`#RRGGBB`の形式で入力してください。", ephemeral=True)
                    return
            ...
            view = ChannelSelectView(self.cog, embed)
            await interaction.followup.send("✅ プレビューが作成されました。下のメニューからメッセージを送信するチャンネルを選択してください。", embed=embed, view=view, ephemeral=True)

        except Exception as e:
            logger.error(f"임베드 생성 중 오류: {e}", exc_info=True)
            await interaction.followup.send("❌ 埋め込みの作成中にエラーが発生しました。", ephemeral=True)


# 2. 임베드를 전송할 채널을 선택하는 View
class ChannelSelectView(ui.View):
    ...
    async def on_channel_select(self, interaction: discord.Interaction):
        await interaction.response.defer(ephemeral=True)
        target_channel = interaction.data['values'][0]
        channel: discord.TextChannel = interaction.guild.get_channel(int(target_channel))
        ...
        try:
            ...
            for item in self.children:
                item.disabled = True
            await interaction.edit_original_response(content=f"✅ <#{channel.id}> にメッセージを正常に送信しました。", view=self)
...
class CustomEmbedPanelView(ui.View):
    ...
    async def on_button_click(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_modal(EmbedCreatorModal(self.cog))
