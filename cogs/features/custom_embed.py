...
class EmbedCreatorModal(ui.Modal, title="カスタム埋め込み作成"):
    ...
    description_input = ui.TextInput(label="説明", placeholder="埋め込みの説明を入力してください。", style=discord.TextStyle.paragraph, required=False, max_length=4000)
    ...
    def __init__(self, cog: 'CustomEmbed', existing_embed: Optional[discord.Embed] = None):
        ...
        if existing_embed:
            ...
            if existing_embed.image and existing_embed.image.url:
                self.image_url_input.default = existing_embed.image.url
            if existing_embed.thumbnail and existing_embed.thumbnail.url:
    ...
    async def on_submit(self, interaction: discord.Interaction):
        if not self.title_input.value and not self.description_input.value and not self.image_url_input.value:
            await interaction.response.send_message("❌ エラー: タイトル、説明、画像URLのいずれか一つは必ず入力する必要があります。", ephemeral=True)
            return
        ...
        try:
            ...
            if self.color_input.value:
                try:
                    ...
                    await interaction.response.send_message("❌ 色コードが無効です。#RRGGBB形式で入力してください。", ephemeral=True)
                    return
            ...
            if not interaction.response.is_done():
                await interaction.response.send_message("❌ 埋め込みを作成中にエラーが発生しました。", ephemeral=True)
...
class ChannelSelectView(ui.View):
    ...
    async def on_channel_select(self, interaction: discord.Interaction):
        ...
        if not channel:
            return await interaction.followup.send("❌ チャンネルが見つかりません。", ephemeral=True)
        ...
        try:
            ...
            for item in self.children:
                item.disabled = True
            await interaction.edit_original_response(content=f"✅ <#{channel.id}> チャンネルにメッセージを正常に送信しました。", view=self, embed=None)

        except Exception as e:
            logger.error(f"커스텀 임베드 전송 중 오류: {e}", exc_info=True)
            await interaction.followup.send(f"❌ <#{channel.id}> チャンネルにメッセージを送信中にエラーが発生しました。", ephemeral=True)
...
class CustomEmbed(commands.Cog):
    ...
    async def edit_embed_message(self, interaction: discord.Interaction, message: discord.Message):
        ...
        if message.author.id != self.bot.user.id or not message.embeds:
            return await interaction.response.send_message("❌ ボットが送信した埋め込みメッセージにのみ使用できます。", ephemeral=True)
        ...
        if modal.embed:
            try:
                ...
                await interaction.followup.send("✅ 埋め込みメッセージを正常に修正しました。", ephemeral=True)
            except Exception as e:
                logger.error(f"임베드 수정 중 오류: {e}", exc_info=True)
                await interaction.followup.send("❌ メッセージを修正中にエラーが発生しました。", ephemeral=True)
