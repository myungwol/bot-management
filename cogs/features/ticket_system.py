...
class InquiryModal(ui.Modal, title="お問い合わせ"):
    ...
    def __init__(self, cog: 'TicketSystem'):
        ...
        # 문의 담당 역할 목록을 가져와서 제외 선택 메뉴를 추가
        inquiry_roles = [
            role for role_id in self.cog.inquiry_role_ids
            if (role := self.cog.bot.get_guild(self.cog.guild_id).get_role(role_id)) is not None
        ]
        if inquiry_roles:
...
class TicketControlView(ui.View):
    ...
    async def delete(self, interaction: discord.Interaction, button: ui.Button):
        ...
        try:
            ...
            pass # 이미 삭제된 경우 무시
        # on_thread_delete 리스너가 데이터 정리를 처리
...
class TicketPanelView(ui.View):
    ...
    @ui.button(label="お問い合わせ", style=discord.ButtonStyle.primary, emoji="📨", custom_id="ticket_inquiry")
    async def inquiry(self, interaction: discord.Interaction, button: ui.Button):
        ...
        if modal.is_submitted():
            excluded_role_ids = [int(role_id) for role_id in modal.exclude_select.values]
...
class TicketSystem(commands.Cog):
    ...
    async def on_thread_delete(self, thread):
        if thread.id in self.tickets:
            ...
            # [신규] 스레드가 삭제되면 부모 포럼 채널도 함께 삭제
            if isinstance(thread.parent, discord.ForumChannel):
                try:
                    await thread.parent.delete(reason="チケットのスレッドが削除されたため")
                except discord.NotFound:
                    pass
    ...
    async def regenerate_panel(self, channel: discord.TextChannel):
        ...
        logger.info(f"✅ 티켓 패널을 #{channel.name} 채널에 생성했습니다.")
