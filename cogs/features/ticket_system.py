...
class StaffApplicationModal(ui.Modal, title="관리자 지원서"):
    ...
    def __init__(self, cog: 'TicketSystem', department_key: str, department_label: str):
        super().__init__()
        self.cog = cog; self.department_key = department_key; self.department_label = department_label

    async def on_submit(self, interaction: discord.Interaction):
        await interaction.response.defer(ephemeral=True)
        try:
            application_data = {
                "지원 부서": self.department_label, "이름 / 나이 / 성별": self.name.value,
                "지원 부서 경력": self.experience.value, "현재 타섭 관리진 유/무": self.other_server_staff.value,
                "주 활동 시간대": self.activity_time.value, "각오": self.resolve.value,
            }
            target_roles = set(self.cog.master_roles)
            await self.cog.create_ticket(
                interaction=interaction, ticket_type="application", title=f"{interaction.user.display_name}님의 관리자 지원",
                content=application_data, selected_roles=target_roles, embed_key="embed_ticket_staff_application",
...
class ApplicationDepartmentSelectView(ui.View):
    def __init__(self, cog: 'TicketSystem', departments: dict):
        super().__init__(timeout=180); self.cog = cog; self.departments = departments
        self.selected_department_key: Optional[str] = None
        options = [discord.SelectOption(label=info['label'], value=key, description=info['description'], emoji=info.get('emoji')) for key, info in departments.items()]
        ...
        self.proceed_button.callback = self.on_proceed; self.add_item(self.proceed_button)

    async def on_proceed(self, interaction: discord.Interaction):
        if not self.selected_department_key: return
...
class InquiryModal(ui.Modal):
    ...
    def __init__(self, cog: 'TicketSystem', selected_roles: Set[discord.Role]):
        super().__init__(title="문의/건의 내용 입력"); self.cog = cog; self.selected_roles = selected_roles
    async def on_submit(self, interaction: discord.Interaction):
        await interaction.response.defer(ephemeral=True)
...
class ReportModal(ui.Modal, title="신고 내용 입력"):
    ...
    def __init__(self, cog: 'TicketSystem', include_police: bool):
        super().__init__(); self.cog = cog; self.include_police = include_police
    async def on_submit(self, interaction: discord.Interaction):
...
# --- ▼▼▼ [오류 수정] InquiryTargetSelectView 클래스 전체를 재설계 ▼▼▼ ---
class InquiryTargetSelectView(ui.View):
    ...
    async def select_target_callback(self, interaction: discord.Interaction, select: ui.Select):
        ...
        # View에 이미 '팀장 선택 메뉴'가 있는지 찾습니다.
        leader_select_instance = next((item for item in self.children if isinstance(item, SpecificLeaderSelect)), None)

        # 선택에 따라 역할을 설정하고, '팀장 선택 메뉴'를 추가하거나 제거합니다.
        if target_type == "master":
            self.selected_roles = set(self.cog.master_roles)
            if leader_select_instance: self.remove_item(leader_select_instance)
            ...
            if not leader_select_instance:
                new_leader_select = SpecificLeaderSelect(self)
                if not new_leader_select.options:
                    await interaction.response.send_message("❌ 현재 문의 가능한 팀장 역할이 설정되지 않았습니다.", ephemeral=True)
                    return
                self.add_item(new_leader_select)

        await interaction.response.edit_message(view=self)

    @ui.button(label="내용 입력하기", style=discord.ButtonStyle.success, row=4)
    async def proceed_button_callback(self, interaction: discord.Interaction, button: ui.Button):
...
class ReportTargetSelectView(ui.View):
    ...
    async def include_police(self, interaction: discord.Interaction, button: ui.Button): await interaction.response.send_modal(ReportModal(self.cog, include_police=True)); await interaction.delete_original_response()
    @ui.button(label="❌ 포장 관리팀 제외하기", style=discord.ButtonStyle.danger)
    async def exclude_police(self, interaction: discord.Interaction, button: ui.Button): await interaction.response.send_modal(ReportModal(self.cog, include_police=False)); await interaction.delete_original_response()
...
class TicketControlView(ui.View):
    ...
    async def _check_handler_permission(self, interaction: discord.Interaction, ticket_type: str) -> bool:
        # 이 유저가 해당 티켓 타입의 담당자인지 확인
        if not isinstance(interaction.user, discord.Member): return False
        handler_roles = set()
        if ticket_type == "report":
            handler_roles.update(self.cog.report_roles)
        elif ticket_type == "inquiry":
            # 문의 티켓은 리더 역할 그룹이 처리
            handler_roles.update(self.cog.leader_roles)
        # 여기에 다른 티켓 타입에 대한 담당자 역할 그룹을 추가할 수 있습니다.
        return not handler_roles.isdisjoint(interaction.user.roles)

    # ▼▼▼▼▼ [핵심 수정] toggle_lock 메소드 전체를 아래 내용으로 교체합니다. ▼▼▼▼▼
    async def toggle_lock(self, interaction: discord.Interaction):
        can_lock = await self._check_master_permission(interaction) or await self._check_handler_permission(interaction, self.ticket_type)
        if not can_lock:
            return await interaction.response.send_message("❌ 이 티켓을 조작할 권한이 없습니다.", ephemeral=True)
        
        thread = interaction.channel
        if not isinstance(thread, discord.Thread) or not (ticket_info := self.cog.tickets.get(thread.id)):
            return await interaction.response.send_message("❌ 이 티켓의 정보를 찾을 수 없습니다.", ephemeral=True)
        ...
        try:
            if is_currently_locked:
                ...
                await interaction.followup.send("✅ 티켓의 잠금을 해제했습니다.", ephemeral=True)
                new_view = TicketControlView(self.cog, self.ticket_type, is_locked=False)
            else:
                # --- 잠금 로직 ---
                all_admin_roles = set(self.cog.master_roles + self.cog.leader_roles + self.cog.report_roles + self.cog.department_manager_roles)
                all_admin_role_ids = {role.id for role in all_admin_roles if role}
                
                # [버그 수정] thread.fetch_members() 앞에 await를 추가하고, 일반 for 루프 사용
                members_in_thread = await thread.fetch_members()
                
                members_to_remove = []
                for member in members_in_thread:
                    if not member.bot and all_admin_role_ids.isdisjoint({r.id for r in member.roles}):
                ...
                await interaction.followup.send("✅ 관리자 외의 멤버를 제외하고 티켓을 잠갔습니다.", ephemeral=True)
                new_view = TicketControlView(self.cog, self.ticket_type, is_locked=True)
            
            # 메시지를 찾아서 view를 업데이트
            original_response = await interaction.original_response()
            await original_response.edit(view=new_view)

        except Exception as e:
            logger.error(f"티켓 잠금/해제 중 오류 발생: {e}", exc_info=True)
            if not interaction.response.is_done():
                await interaction.followup.send("❌ 티켓 상태 변경 중 오류가 발생했습니다.", ephemeral=True)
    # ▲▲▲▲▲ [수정 완료] ▲▲▲▲▲
...
class TicketSystem(commands.Cog):
    def __init__(self, bot: commands.Bot):
        ...
        self.department_manager_roles: List[discord.Role] = []; self.guild: Optional[discord.Guild] = None
        self.view_instance: Optional[MainTicketPanelView] = None; self.departments: Dict[str, Any] = {}
    ...
    async def sync_tickets_from_db(self):
        ...
        await self.load_configs() # DB 동기화 전에 설정부터 로드
