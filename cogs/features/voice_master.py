...
# ▼▼▼ [수정] 새로운 채널 타입 정보로 완전히 교체합니다. ▼▼▼
# user_limit: 0은 무제한을 의미합니다. 필요시 이 값을 수정하세요.
CHANNEL_TYPE_INFO = {
    "mixer":  {"emoji": "🧊", "name_editable": False, "limit_editable": False, "default_name": "소형믹서", "user_limit": 2},
    "line":   {"emoji": "🔧", "name_editable": False, "limit_editable": False, "default_name": "미니라인", "user_limit": 4},
    "sample": {"emoji": "⚙️", "name_editable": False, "limit_editable": False, "default_name": "샘플룸",  "user_limit": 6},
...
class VCOwnerSelect(ui.UserSelect):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if not isinstance(new_owner, discord.Member) or new_owner.id == self.panel_view.owner_id or new_owner.bot:
            return
...
class ControlPanelView(ui.View):
    ...
    def setup_buttons(self):
        self.clear_items()
        # 게임방에만 필요한 '이름 변경'과 '소유권 이전' 버튼만 추가합니다.
    ...
    async def dispatch_button(self, interaction: discord.Interaction):
        ...
        dispatch_map = {"vc_edit": self.edit_channel, "vc_transfer": self.transfer_owner}
    ...
    async def edit_channel(self, interaction: discord.Interaction):
        ...
        # 이름에서 접두사를 제외한 순수 이름만 추출
        current_name = vc.name.split('ㆍ')[-1].strip()
        modal = VCEditModal(current_name=current_name)
        ...
        if modal.submitted:
            ...
            msg = await interaction.followup.send("✅ 채널 이름을 업데이트했습니다.", ephemeral=True)
...
class VoiceMaster(commands.Cog):
    ...
    async def load_configs(self):
        # ▼▼▼ [수정] 새로운 DB 키를 사용하도록 변경 ▼▼▼
        self.creator_channel_configs = {
            get_id("vc_creator_mixer"): {"type": "mixer"},
            get_id("vc_creator_line"): {"type": "line"},
            get_id("vc_creator_sample"): {"type": "sample"},
            get_id("vc_creator_game"): {"type": "game"},
        }
    ...
    async def sync_channels_from_db(self):
        ...
        for ch_data in db_channels:
            ...
            if guild and guild.get_channel(channel_id):
                self.temp_channels[channel_id] = ch_data
                self.user_channel_map[owner_id] = channel_id
                
                # 게임방만 제어판 View를 등록합니다.
                if ch_data.get("channel_type") == "game":
                    message_id = ch_data.get("message_id")
                    view = ControlPanelView(self, owner_id, channel_id, "game")
    ...
    async def _send_control_panel(self, vc: discord.VoiceChannel, owner: discord.Member) -> discord.Message:
        embed = discord.Embed(
            title=f"ようこそ、{get_clean_display_name(owner)}様！",
            description="ここはあなたの個人チャンネルです。\n下記ボタンでチャンネルを管理できます。",
            color=0x7289DA
        )
        view = ControlPanelView(self, owner.id, vc.id, "game")
        return await vc.send(f"{owner.mention}", embed=embed, view=view, allowed_mentions=discord.AllowedMentions(users=True))
    ...
    async def _transfer_ownership(self, interaction: discord.Interaction, vc: discord.VoiceChannel, new_owner: discord.Member):
        ...
        try:
            ...
            self.user_channel_map.pop(old_owner_id, None)
            ...
            new_view = ControlPanelView(self, new_owner.id, vc.id, "game")
