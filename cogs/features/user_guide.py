...
from utils.helpers import format_embed_from_db, has_required_roles # [수정] has_required_roles import
from utils.ui_defaults import AGE_ROLE_MAPPING_BY_YEAR # [수정] 새로운 나이 역할 매핑 import
...
class GuideThreadView(ui.View):
    ...
    async def _update_view_state(self, interaction: discord.Interaction, new_page: int, total_pages: int):
    ...
    async def go_previous(self, interaction: discord.Interaction, button: ui.Button):
        ...
        await self._update_view_state(interaction, new_page, len(steps))
...
class UserGuidePanelView(ui.View):
    ...
    async def start_guide_callback(self, i: discord.Interaction):
        ...
        try:
            ...
            # [수정] Private Thread가 아닌 Public Thread로 생성하여 스태프가 볼 수 있도록 함
            ...
            await guide_view._update_view_state(i, 0, len(steps))
            
            initial_embed = format_embed_from_db(steps[0], user_mention=i.user.mention)
            
            # [수정] 초기 메시지에서 스태프 멘션 제거
            await thread.send(
                content=f"{i.user.mention}", 
                embed=initial_embed, 
                view=guide_view, 
                allowed_mentions=discord.AllowedMentions(users=True, roles=False) # roles=False
            )
