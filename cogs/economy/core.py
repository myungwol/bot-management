...
class EconomyCore(commands.Cog):
    ...
    async def transfer_command(self, interaction: discord.Interaction, recipient: discord.Member, amount: app_commands.Range[int, 1, None]):
        ...
        if sender_wallet.get('balance', 0) < amount:
            return await interaction.response.send_message(f"残高が不足しています。", ephemeral=True)
    ...
    async def give_coin_command(self, interaction: discord.Interaction, user: discord.Member, amount: int):
        ...
        if result:
            ...
            embed.set_footer(text=f"付与後の残高: {result['balance']:,} {CURRENCY_ICON} | 処理者: {interaction.user.display_name}")
            await interaction.response.send_message(embed=embed)
        else:
    ...
    async def take_coin_command(self, interaction: discord.Interaction, user: discord.Member, amount: int):
        if not self.admin_role_id or not discord.utils.get(interaction.user.roles, id=self.admin_role_id):
            return await interaction.response.send_message("このコマンドを使用する権限がありません。", ephemeral=True)
        result = await update_wallet(user, -amount)
        if result:
            ...
            embed.set_footer(text=f"削減後の残高: {result['balance']:,} {CURRENCY_ICON} | 処理者: {interaction.user.display_name}")
            await interaction.response.send_message(embed=embed)
        else:
