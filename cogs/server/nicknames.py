# cogs/server/nicknames.py (최종 수정본 - 특수문자/이모티콘 금지 기능 추가)
...
from utils.database import get_panel_id, save_panel_id, get_cooldown, set_cooldown, get_channel_id_from_db, get_role_id

# [핵심] 허용된 문자만으로 구성되었는지 확인하는 정규표현식
# 한자, 히라가나, 가타카나, 영문 알파벳, 숫자만 허용
...
class NicknameApprovalView(ui.View):
    ...
    @ui.button(label="承認", style=discord.ButtonStyle.success, custom_id="nick_approve_v5")
    async def approve(self, interaction: discord.Interaction, button: ui.Button):
        ...
        try:
            ...
            logger.error(f"Error during nickname approval: {e}", exc_info=True)
            return
    ...
    @ui.button(label="拒否", style=discord.ButtonStyle.danger, custom_id="nick_reject_v5")
...
class NicknameChangeModal(ui.Modal, title="名前変更申請"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if not ALLOWED_NICKNAME_PATTERN.match(new_name_value):
            await interaction.followup.send("❌ エラー: 名前に絵文字や特殊文字は使用できません。", ephemeral=True)
            return
        ...
        if weighted_length > 8:
            error_message = (
                f"❌ エラー: 名前の長さがルールを超えています。\n"
                f"ルール: 合計8文字まで (漢字は2文字として計算)\n"
                f"現在の文字数: **{weighted_length}/8**"
            )
        ...
        if not nicknames_cog or not nicknames_cog.approval_channel_id or not nicknames_cog.approval_role_id:
            await interaction.followup.send("エラー: ニックネーム機能が正しく設定されていません。", ephemeral=True)
            return
        ...
        await approval_channel.send(
            content=f"<@&{self.approval_role_id}>",
            embed=embed,
...
class NicknameChangerPanelView(ui.View):
    ...
    @ui.button(label="名前変更申請", style=discord.ButtonStyle.primary, custom_id="nickname_change_button_v6")
    async def request_change(self, interaction: discord.Interaction, button: ui.Button):
        ...
        if last_request_time and time.time() - last_request_time < COOLDOWN_SECONDS:
            ...
            await interaction.response.send_message(f"次の申請まであと {remaining_td_str} お待ちください。", ephemeral=True)
            return

        nicknames_cog = interaction.client.get_cog("Nicknames")
        if not nicknames_cog or not nicknames_cog.approval_role_id:
            await interaction.response.send_message("エラー: ニックネーム変更機能が設定されていません。", ephemeral=True)
            return
...
class Nicknames(commands.Cog):
    ...
    async def update_nickname(self, member: discord.Member, base_name_override: str = None, force_update: bool = False):
        if member.bot or member.id == member.guild.owner_id or member.guild.me.top_role <= member.top_role:
            return
        ...
        if base_name is None:
            current_display_name = member.nick if member.nick else member.name
        ...
        if force_update or member.nick != new_nickname:
            try:
                ...
                logger.warning(f"Failed to update nickname for {member.name}: Missing Permissions.")
            except Exception as e:
                logger.error(f"Error during nickname change for {member.name}: {e}", exc_info=True)
    ...
    async def on_member_update(self, before: discord.Member, after: discord.Member):
        ...
        if before.nick != after.nick:
            logger.info(f"User {after.name} changed nickname from '{before.nick}' to '{after.nick}'. Re-applying rules.")
            await self.update_nickname(after, force_update=True)

        elif before.roles != after.roles:
            logger.info(f"Roles updated for {after.display_name}, triggering nickname update.")
    ...
    async def on_member_join(self, member: discord.Member):
        ...
        logger.info(f"New member {member.name} joined. Applying initial nickname.")
    ...
    async def setup_nickname_panel_command(self, i: discord.Interaction):
        ...
        if i.channel.id != self.panel_and_result_channel_id:
            await i.response.send_message(f"このコマンドは<#{self.panel_and_result_channel_id}>でのみ使用できます。", ephemeral=True)
            return
