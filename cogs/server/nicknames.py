# cogs/server/nicknames.py (UI 중앙 관리 시스템 연동)
...
# [수정] panel_manager Cog는 더 이상 직접 임포트할 필요가 없습니다.
# 대신, 다른 파일과 마찬가지로 버튼 스타일 맵을 직접 정의하거나 utils 같은 곳으로 옮기는 것이 좋습니다.
# 여기서는 간단하게 직접 정의합니다.
BUTTON_STYLES_MAP = {
...
class Nicknames(commands.Cog):
    ...
    async def get_final_nickname(self, member: discord.Member, base_name: str = "") -> str:
        prefix = None; member_role_names = {role.name for role in member.roles}
        ...
        if base_name.strip(): base = base_name.strip()
            ...
            for p_name in NICKNAME_PREFIX_HIERARCHY_NAMES:
                if current_nick.startswith(f"[{p_name}]"): base = current_nick[len(p_name) + 2:].lstrip(); break
        final_nick = f"{prefix}{base}" if prefix else base
        if len(final_nick) > 32:
            prefix_len = len(prefix) if prefix else 0; base = base[:32 - prefix_len]
            final_nick = f"{prefix}{base}" if prefix else base
        return final_nick
    ...
    async def regenerate_panel(self, channel: Optional[discord.TextChannel] = None):
        ...
        # [수정] 패널 임베드를 DB에서 불러옵니다.
        embed_data = await get_embed_from_db("panel_nicknames")
        if not embed_data:
            logger.warning("DB에서 'panel_nicknames' 임베드 데이터를 찾을 수 없어, 패널 생성을 건너뜁니다."); return
