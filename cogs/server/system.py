# cogs/server/system.py (명령어 최적화 적용 최종본)
...
from utils.database import (
    ...
    delete_all_buttons_for_panel, bulk_add_auto_role_buttons, # 최적화 함수 임포트
...
class EphemeralRoleSelectView(ui.View):
    def __init__(self, member: discord.Member, category_roles: list, all_category_role_ids: set[int]):
        super().__init__(timeout=180); self.member = member; self.all_category_role_ids = all_category_role_ids
...
class EmbedEditModal(ui.Modal, title="임베드 내용 편집"):
    def __init__(self, embed: discord.Embed): super().__init__(); self.embed = embed
    ...
    async def on_submit(self, i: discord.Interaction): self.embed.title = self.embed_title.value; self.embed.description = self.embed_description.value.replace('\\n', '\n'); await i.response.edit_message(embed=self.embed)

class EmbedEditorView(ui.View):
    def __init__(self, msg: discord.Message, key: str): super().__init__(timeout=None); self.message = msg; self.embed_key = key
    ...
    async def save_to_db(self, i: discord.Interaction, b: ui.Button): await i.response.defer(ephemeral=True); await save_embed_to_db(self.embed_key, self.message.embeds[0].to_dict()); await i.followup.send(f"✅ DB에 '{self.embed_key}'로 저장됨.", ephemeral=True)
...
class ServerSystem(commands.Cog):
    def __init__(self, bot: commands.Bot): self.bot = bot; self.welcome_channel_id: Optional[int] = None; self.farewell_channel_id: Optional[int] = None; self.temp_user_role_id: Optional[int] = None; self.counter_configs = []; self.update_tasks: dict[int, asyncio.Task] = {}; logger.info("ServerSystem Cog initialized.")
    async def cog_load(self): await self.load_all_configs(); self.update_counters_loop.start()
    def cog_unload(self): self.update_counters_loop.cancel(); [task.cancel() for task in self.update_tasks.values()]
    async def load_all_configs(self): self.welcome_channel_id = await get_channel_id_from_db("welcome_channel_id"); self.farewell_channel_id = await get_channel_id_from_db("farewell_channel_id"); self.temp_user_role_id = get_role_id("temp_user_role"); self.counter_configs = await get_counter_configs(); logger.info("[ServerSystem Cog] Loaded all configurations.")
    
    # ... (카운터, 이벤트 리스너 함수들은 이전과 동일하므로 여기에 그대로 복사) ...
        ...
        for panel_key, panel_config in STATIC_AUTO_ROLE_PANELS.items():
            try:
                ...
                if not (channel := interaction.guild.get_channel(channel_id)):
                    logger.warning(f"패널 '{panel_key}' 배포 실패: 채널을 찾을 수 없습니다.")
                    continue
                ...
                if panel_message_id:
                    try:
                        live_message = await channel.fetch_message(panel_message_id)
                ...
                # [최적화] 봇 재시작 시 View 복구를 위한 DB 작업을 효율적으로 처리
    ...
    async def create_message_editor(self, i: discord.Interaction, ch: discord.TextChannel, key: str, name: str):
        await i.response.defer(ephemeral=True)
        embed = discord.Embed.from_dict(await get_embed_from_db(key) or {"title":f"{name} 제목", "description":f"{name} 설명"})
