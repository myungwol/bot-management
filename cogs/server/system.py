...
# ---------------------------------------------------------------------------------------------
# [일회용 서버 초기화 코드] - 이 아래 부분을 system.py 파일 하단에 추가하세요.
# ---------------------------------------------------------------------------------------------
...
ROLE_STRUCTURE = {
    "💎 관리팀": [
        {"name": "촌장", "color": 0xFFD700},
        {"name": "부촌장", "color": 0xC0C0C0},
        {"name": "직원", "color": 0xB2B2B2},
        {"name": "경찰관", "color": 0x3498DB},
        {"name": "축제 담당", "color": 0xE91E63},
        {"name": "홍보 담당", "color": 0x2ECC71},
        {"name": "마을 디자이너", "color": 0x9B59B6},
        {"name": "서기", "color": 0x71368A},
        {"name": "도우미", "color": 0x1ABC9C},
    ],
    "✨ 특별 역할": [
        {"name": "후원자", "color": 0xF47FFF},
    ],
    "📈 주민 등급": [
        {"name": "장로", "color": 0x99AAB5},
        {"name": "베테랑 주민", "color": 0x607D8B},
        {"name": "단골 주민", "color": 0x7289DA},
        {"name": "새내기 주민", "color": 0x979C9F},
        {"name": "주민", "color": 0x22A669},
        {"name": "여행객", "color": 0x83909F},
    ],
    "🎣 직업 역할": [
        {"name": "강태공", "color": 0x206694},
        {"name": "대농", "color": 0x4E2C2C},
        {"name": "낚시꾼", "color": 0xADD8E6},
        {"name": "농부", "color": 0x964B00},
    ],
    "🎨 선택 역할": [
    ...
    "⚙️ 시스템 역할": [
        {"name": "숲의 요정", "color": 0x2ECC71}, # 봇 역할
        {"name": "경고 1회", "color": 0xFEE75C},
        {"name": "경고 2회", "color": 0xE67E22},
        {"name": "경고 3회", "color": 0xED4245},
        {"name": "경고 4회", "color": 0x992D22},
...
class ServerSystem(commands.Cog):
    # ... 기존 ServerSystem Cog 코드는 여기에 그대로 둡니다 ...
    
    # --- 이 아래에 새로운 명령어를 추가합니다 ---

    async def perform_server_initialization(self, interaction: discord.Interaction):
        """서버 초기화 로직을 수행하는 메인 함수"""
        ...
        for category_name, channel_list in SERVER_STRUCTURE.items():
            ...
            for channel_name in channel_list:
                ...
                if is_voice:
                    channel_name = channel_name.replace("VOICE:", "", 1)
                ...
                try:
                    if is_voice:
                        await target_category.create_voice_channel(name=channel_name, reason="서버 초기화")
                    else:
                        await target_category.create_text_channel(name=channel_name, reason="서버 초기화")
...
# ---------------------------------------------------------------------------------------------
# [일회용 서버 초기화 코드] - setup 함수 바로 위에 위치해야 합니다.
# ---------------------------------------------------------------------------------------------
