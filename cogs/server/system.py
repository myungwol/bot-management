...
class ServerSystem(commands.Cog):
    ...
    async def setup_action_autocomplete(self, interaction: discord.Interaction, current: str) -> List[app_commands.Choice[str]]:
        ...
        extended_admin_map["boost_test"] = "[부스트] 부스트 보상 지급 테스트 (1회)"
    ...
    async def setup(self, interaction: discord.Interaction, action: str,
        ...
        if action == "status_show":
            ...
            tasks = [save_config_to_db(f"panel_regenerate_request_{panel_key}", timestamp) for panel_key in game_panel_keys]
                ...
                if info.get("type") == "panel":
                    ...
                    try:
                        ...
                        if not all([cog_name, channel_db_key]):
                            failure_list.append(f"・`{friendly_name}`: 설정 정보가 불완전합니다."); continue
                        ...
                        if is_game_panel:
                            ...
                            success_list.append(f"・`{friendly_name}`: 게임 봇에게 재설치를 요청했습니다."); continue
                        cog = self.bot.get_cog(cog_name)
                        if not cog or not hasattr(cog, 'regenerate_panel'):
                            failure_list.append(f"・`{friendly_name}`: Cog를 찾을 수 없거나 재설치 기능이 없습니다."); continue
                        channel_id = get_id(channel_db_key)
                        if not channel_id or not (target_channel := self.bot.get_channel(channel_id)):
                            failure_list.append(f"・`{friendly_name}`: 채널이 설정되지 않았거나 찾을 수 없습니다."); continue
            ...
            if stat_type == "remove":
                ...
                if "{count}" not in current_template: return await interaction.followup.send("❌ 이름 형식(`template`)에는 반드시 `{count}`를 포함해야 합니다.", ephemeral=True)
                if stat_type == "role" and not role: return await interaction.followup.send("❌ '특정 역할 멤버 수'는 `role` 옵션을 지정해야 합니다.", ephemeral=True)
