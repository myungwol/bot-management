...
class MemberEvents(commands.Cog):
    def __init__(self, bot: commands.Bot):
        ...
        # 부스트 로그 채널 ID를 저장할 변수 추가
        self.boost_log_channel_id: Optional[int] = None
    ...
    async def load_configs(self):
        ...
        # 부스트 로그 채널 ID를 DB 캐시에서 불러옵니다.
        self.boost_log_channel_id = get_id("boost_log_channel_id")
    ...
    async def on_member_update(self, before: discord.Member, after: discord.Member):
        ...
        # 부스트 알림을 보낼 채널 가져오기
        boost_channel = self.bot.get_channel(self.boost_log_channel_id) if self.boost_log_channel_id else None
        ...
        if before.premium_since is None and after.premium_since is not None:
            ...
            roles_to_add_keys = []
            if num_existing_tickets < 10: roles_to_add_keys.append(f"role_boost_ticket_{num_existing_tickets + 1}")
            if num_existing_tickets + 1 < 10: roles_to_add_keys.append(f"role_boost_ticket_{num_existing_tickets + 2}")
            ...
            try:
                ...
                if boost_channel:
                    ...
                    if embed_data:
                        ...
                        embed = format_embed_from_db(embed_data, member_mention=after.mention, guild_name=after.guild.name, roles_list=roles_list_str)
                ...
                logger.error(f"{after.display_name}님에게 부스트 보상 역할을 지급하거나 채널에 메시지를 보내지 못했습니다. (권한 부족)")
            except Exception as e:
        ...
        # --- 시나리오 2: 사용자가 부스트를 중지했을 때 ---
            ...
            try:
                ...
                # 설정된 채널로 부스트 중지 알림 발송
                if boost_channel:
                    embed_data = await get_embed_from_db("log_boost_stop")
                    if embed_data:
                        embed = format_embed_from_db(embed_data, member_mention=after.mention, guild_name=after.guild.name)
                        if after.display_avatar:
                            embed.set_author(name=after.display_name, icon_url=after.display_avatar.url)
