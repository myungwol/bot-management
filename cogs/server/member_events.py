...
# [✅ 수정] supabase 클라이언트를 직접 사용하기 위해 import
...
class MemberEvents(commands.Cog):
    ...
    async def on_member_join(self, member: discord.Member):
        ...
        # --- [✅ 핵심 수정] ---
        # 새로운 멤버의 초기 레벨 데이터를 DB에 생성합니다.
        # upsert는 데이터가 이미 존재하면 아무 작업도 하지 않으므로 안전합니다.
        try:
            ...
            logger.info(f"'{member.display_name}'님의 초기 레벨 데이터를 DB에 생성/확인했습니다.")
...
#### 2. bot-management/cogs/server/system.py
`_update_user_xp_and_level` 함수에서 DB 조회 결과가 `None`이거나 `res.data`가 비어있는 경우를 모두 안전하게 처리하도록 수정했습니다.
